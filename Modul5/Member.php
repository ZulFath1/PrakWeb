...
<head>
    ...
    <title>Data Member - Perpustakaan OPM</title>
        ...
        <div class="column has-background-dark is-flex is-flex-direction-column">
            ...
            <nav class="top-navbar">
                <div class="has-text-grey-light is-size-6">
                    Dashboard / <span class="has-text-weight-bold has-text-white">Member</span>
                </div>
            ...
            <div class="content-area flex-grow-1">
                ...
                <div class="custom-box">
                    ...
                    <div class="px-5 py-4 is-flex is-justify-content-space-between is-align-items-center" style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                        <div>
                            <h2 class="title is-4 mb-1 has-text-weight-bold has-text-white">Direktori Member</h2>
                            <p class="subtitle is-6 has-text-grey-light mt-1">Kelola data keanggotaan dan masa aktif member.</p>
                        </div>
                        <a href="FormTambahMember.php" class="button is-primary has-text-weight-medium" style="border-radius: 8px;">
                            <i class="fa-solid fa-plus mr-2"></i> Tambah Data Member
                        </a>
                    </div>
                    ...
                    <div class="p-2 table-container">
                        <table class="table is-fullwidth is-hoverable has-background-transparent">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="has-text-grey-light pl-5">#<?= $row['id_member'] ?></td>
                                    <td class="has-text-weight-medium has-text-white"><?= $row['nama_member'] ?></td>
                                    <td><span class="tag is-gray border is-rounded"><?= $row['nomor_member'] ?></span></td>
                                    <td class="has-text-grey-light"><?= $row['alamat'] ?></td>
                                    <td class="has-text-grey-light"><?= date('d M Y, H:i', strtotime($row['tgl_mendaftar'])) ?></td>
                                    <td class="has-text-grey-light"><?= date('d M Y', strtotime($row['tgl_terakhir_bayar'])) ?></td>
                                    <td class="has-text-right pr-5">
                                        <a href="FormEditMember.php?id=<?= $row['id_member'] ?>" class="action-link has-text-primary mr-3">
                                            <i class="fa-solid fa-pen mr-1"></i> Edit
                                        </a>
                                        <a href="Member.php?id_hapus=<?= $row['id_member'] ?>" class="action-link has-text-danger" onclick="return confirm('Apakah Anda yakin ingin menghapus member ini?');">
